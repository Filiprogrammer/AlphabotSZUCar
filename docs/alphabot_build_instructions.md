...
![Positioning module assembly 1](positioning_module_assembly_1.png)

Now solder pin headers for the Arduino Pro Mini to the board as seen in the following picture.

![Positioning module assembly 2](positioning_module_assembly_2.png)

Finally plug the Arduino Pro Mini into the DWM1000 Adapter.

![Positioning module assembly 3](positioning_module_assembly_3.png)
...
![Installing PlatformIO in VSCode](vscode_platformio_install.png)
...
![Aduino Pro Mini connected through USB to TTL adapter](usb_to_ttl_adapter.jpg)

The firmware can be uploaded to the Arduino Pro Mini by clicking the upload button in VSCode.

![VSCode PlatformIO upload](vscode_platformio_upload.png)
...
![VSCode PlatformIO serial monitor](vscode_platformio_serial_monitor.png)

If the process was successful, the following message will appear in the Serial Monitor:

![Serial output of positioning tag](positioning_tag_serial_output.png)
...
![Positioning anchor address](positioning_anchor_address.png)
...
![Labeled positioning modules](positioning_modules_label.jpg)

Once the firmware is uploaded, the serial monitor should display the following message with the corresponding address:

![Serial output of positioning anchor](positioning_anchor_serial_output.png)
...
![Positioning anchor with a holder](positioning_anchor_with_holder.png)

Screw the positioning module on with two M2x6 screws and two M2 nuts.

![Positioning anchor mounted to a holder](positioning_anchor_mounted.jpg)
...
![Alphabot ESP32 PCB](alphabot_esp32_pcb.jpg)
...
![Alphabot ESP32 PCB assembly 1](alphabot_esp32_pcb_assembly_1.png)

Also solder two S5B-XH-A connector headers for the stepper motors to the board as seen in the following picture.

![Alphabot ESP32 PCB assembly 2](alphabot_esp32_pcb_assembly_2.png)

Solder a CZM5/2 terminal block to the 12V power pins of the board.

![Alphabot ESP32 PCB assembly 3](alphabot_esp32_pcb_assembly_3.png)

Finally you can plug the ESP-WROOM-32 into the Alphabot ESP32 PCB.

![Alphabot ESP32 PCB assembly 4](alphabot_esp32_pcb_assembly_4.png)
...
![Solder wires to the contacts of the drive motor](gear_motor_solder_wires.jpg)
...
![Insert gear motors into the base plate](base_plate_insert_gear_motors.png)
...
![Attach a rear wheel 3](attach_rear_wheel_3.png)
...
![Attach a rear wheel 4](attach_rear_wheel_4.png)

Attach the rear wheel to the axle.

![Attach a rear wheel 5](attach_rear_wheel_5.png)
...
![Attach a rear wheel 6](attach_rear_wheel_6.png)

Once both rear wheels are attached it should look like this:

![Rear wheels are attached](rear_wheels_attached.png)
...
![Steering bracket 1](steering_bracket_1.png)

![Steering bracket 2](steering_bracket_2.png)
...
![Front bumper 1](front_bumper_1.png)

Attach the LiDAR holder (stl/vehicle/addon/bumper.lidar.holder.stl) to the hub of the stepper motor.

![Front bumper 2](front_bumper_2.png)

In the next step, screw the steering bracket to the front bumper. Use four DIN 912 M3x5 screws for this.

![Screw the steering bracket to the front bumper 1](screw_steering_bracket_to_front_bumper_1.png)

![Screw the steering bracket to the front bumper 2](screw_steering_bracket_to_front_bumper_2.png)
...
![Attach a steering stepper motor to the base plate 1](attach_steering_stepper_1.png)

![Attach a steering stepper motor to the base plate 2](attach_steering_stepper_2.png)
...
![Attach the steering extension](steering_extension.png)

Now insert the two 5x8x2.5 ball bearings on the steering axle (stl/vehicle/steering/steering.axle.stl).

![Insert ball bearings on the steering axle](steering_axle_insert_ball_bearings.png)
...
![Attach the steering axle & suspenions 1](steering_axle_and_suspensions_1.png)

![Attach the steering axle & suspenions 2](steering_axle_and_suspensions_2.png)

Attach 5x11x4 ball bearings to the underside of the steering axles.

![Attach the steering axle & suspenions 3](steering_axle_and_suspensions_3.png)
...
![Screw base plate and steering bracket together 1](base_plate_steering_bracket_1.png)

![Screw base plate and steering bracket together 2](base_plate_steering_bracket_2.png)

The next step is to mount the front wheels.

![Attach a front wheel 1](attach_front_wheel_1.png)
...
![Attach a front wheel 2](attach_front_wheel_2.png)
...
![Attach a front wheel 3](attach_front_wheel_3.png)
...
![Battery case on the underside of the base plate](battery_case.jpg)

Mount the FC-03 infrared sensor around the encoder wheel with an M2.5 screw.

![FC-03 infrared sensor](fc-03_infrared_sensor.jpg)
...
![Attach the Alphabot ESP32 PCB](attach_alphabot_esp32_pcb.png)

Connect the Alphabot ESP32 PCB to the power supply through its 12V power connectors.

![Connect the Alphabot ESP32 PCB to 12V power](alphabot_esp32_pcb_12v_power.png)

Connect the pins of the L298N motor driver to the Alphabot ESP32 PCB.

![Connect L298N to Alphabot ESP32 PCB](alphabot_esp32_pcb_connect_l298n.png)

Now insert the positioning module configured as a tag into the DWM1000 tag holder.

![Insert the positioning tag into the tag holder](insert_positioning_tag.png)

Connect the GND, VCC and TXO pins on the Arduino Pro Mini of the positioning module to the Alphabot ESP32 PCB.

![Connect Arduino Pro Mini pins to the Alphabot ESP32 PCB](connect_positioning_tag_to_alphabot_esp32_pcb.png)

Screw the positioning module on with two M2x6 screws and two M2 nuts.

![Screw the positioning tag on](positioning_tag_screw.png)

Connect the steering stepper motor to the correspondingly labeled B5B-XH-AM connector header on the Alphabot ESP32 PCB.

![Connect the steering stepper motor to the Alphabot ESP32 PCB](connect_steering_stepper_to_alphabot_esp32_pcb.png)

Connect the stepper motor at the front to the other B5B-XH-AM connector header on the Alphabot ESP32 PCB.

![Connect the front stepper motor to the Alphabot ESP32 PCB](connect_front_stepper_motor_to_alphabot_esp32_pcb.png)

Connect the TF-Luna LiDAR sensor at the front to the Alphabot ESP32 PCB.

![Connect the TF-Luna to the Alphabot ESP32 PCB](connect_tf-luna_to_alphabot_esp32_pcb.png)

Now that the Alphabot is successfully assembled, the software part can begin.

![Assembled Alphabot](alphabot_assembled.png)
